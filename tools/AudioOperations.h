...
class AudioOperations
{
  ...
  public:
	...
	//Creates a new wave file containing only a portion of the
	// original file. Provide where in the original file you
	// would like the new file to begin (in seconds), and provide
	// how long you want the new file to carry on from it's start
	// (in seconds). If you provide a length that exceeds the 
	// end of the original file, the new file will only last
	// until that point.
	static const char* AudioOperations::trim(const char* fileName, double startInSeconds, double lengthInSeconds);

	//Creates a new wave file containing only a portion of the
	// original file. Provide where in the original file you
	// would like the new file to begin (in samples), and provide
	// how long you want the new file to carry on from it's start
	// (in samples). If you provide a length that exceeds the 
	// end of the original file, the new file will only last
	// until that point.
	static const char* AudioOperations::trimSamples(const char* fileName, int startInSamples, int lengthInSamples);

	//Creates a new wave file with silence added at the beginning
	// and/or the end of the original file.
	static const char* AudioOperations::zeroPad(const char* fileName, double startPadInSeconds, double endPadInSeconds);

	//Creates a new wave file by linking the two original files
	// with one another in succession. Parameter fileNameA will
	// specify the file that will come first, and fileNameB will
	// specify the file that will come first second.
	...
	//Creates a new wave file from a given stereo file wherein
	// the stereo channels are switched.
	static const char* AudioOperations::switchStereoChannels(const char* fileName);

	//Creates a new wave file from a given stereo file wherein
	// the polarity of the waveform is flipped. Note that this
	// process yields no immediately audible change.
	...
	// file. Only works on stereo files.
	static const char** AudioOperations::split(const char* fileName);

	//Creates one stereo wave file from two mono wave files.
	// If loop = false, the longer file will be trimmed to be the 
	// same length as the shorter file. If loop = true, the shorter
	// file will be looped until the end of the longer file.
	// Audio format, sample rate, and bit depth must be equal.
	// Only works with mono files.
	static const char* AudioOperations::merge(const char* fileNameL, const char* fileNameR, bool loop);

  private:

	static void AudioOperations::monoSplit(int16_t * samples, int16_t *left, int16_t *right);

	static void AudioOperations::writeHeader(std::ofstream* out, int fileSize, int audioFormat,
		int numChannels, int sampleRate, int byteRate, int blockAlign, int bitsPerSample, int dataSize);

	static void AudioOperations::write16BitSample(std::ofstream* outputFile, int16_t value);

	static void AudioOperations::initialize(const char* fileName);
};